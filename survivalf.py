...
MASS_F_MIN, MASS_F_MAX = 0.5,  20.0
ECC_F_MIN,  ECC_F_MAX  = 0.0,  0.50
...
def run_sim(params):
    ...
    sim.add(m=PLANET_D['m'], a=PLANET_D['a'], e=PLANET_D['e'],
            omega=PLANET_D['omega'], r=PLANET_D['r'])
    sim.add(m=PLANET_E['m'], a=PLANET_E['a'], e=PLANET_E['e'],
            omega=PLANET_E['omega'], r=PLANET_E['r'])
    sim.add(m=mass_f*ME, a=PLANET_F_A, e=ecc_f)
...
if __name__ == "__main__":
    mass_f_vals = np.linspace(MASS_F_MIN, MASS_F_MAX, N_GRID)
    ecc_f_vals  = np.linspace(ECC_F_MIN,  ECC_F_MAX,  N_GRID)

    work_list = [(m, e) for m in mass_f_vals for e in ecc_f_vals]
    total     = len(work_list)

    n_workers = max(1, os.cpu_count() - 1)
    print("LHS 1903 -- Survival Time Grid: Mass of f vs Eccentricity of f")
    print("Grid: " + str(N_GRID) + "x" + str(N_GRID) + " = " + str(total) + " sims")
    print("T = " + str(T_INT) + " yr (~10^6 orbits of planet f) | WHFast | dt = 0.0003 yr")
    ...
    survival_vals = [results_dict[(m, e)] for m in mass_f_vals for e in ecc_f_vals]
    ...
    np.save('survival_mass_f_vs_ecc_f.npy', grid)
    print("Grid saved -- survival_mass_f_vs_ecc_f.npy")

    fig, ax = plt.subplots(figsize=(10, 7))

    img = ax.imshow(
        np.log10(grid + 1),
        extent=[ECC_F_MIN, ECC_F_MAX, MASS_F_MIN, MASS_F_MAX],
        origin='lower',
        cmap='RdYlGn',
        vmin=0,
        vmax=np.log10(T_INT),
    ...
    tick_years = [100, 500, 1000, 5000, 10000, 50000, 147600]
    tick_pos   = [np.log10(t) for t in tick_years]
    cbar.set_ticks(tick_pos)
    cbar.set_ticklabels([str(t) for t in tick_years])

    ax.set_xlabel('Eccentricity of Planet f  ($e_f$)', fontsize=12)
    ax.set_ylabel('Mass of Planet f  ($M_f / M_\\oplus$)', fontsize=12)
    ax.set_title(
        'LHS 1903 -- Survival Time Map: Mass of f vs Eccentricity of f\n'
        'P=53.9d | a=0.2083 AU | WHFast | dt=0.0003 yr | T=147,600 yr | 50x50 | Wilson et al. 2026',
        fontsize=11
    )

    plt.tight_layout()
    plt.savefig('survival_mass_f_vs_ecc_f.png', dpi=300, bbox_inches='tight')
    plt.show()
    print("Saved -- survival_mass_f_vs_ecc_f.png")
